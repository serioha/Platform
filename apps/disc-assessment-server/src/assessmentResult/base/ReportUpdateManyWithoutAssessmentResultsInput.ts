/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ReportWhereUniqueInput } from "../../report/base/ReportWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ReportUpdateManyWithoutAssessmentResultsInput {
  @Field(() => [ReportWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReportWhereUniqueInput],
  })
  connect?: Array<ReportWhereUniqueInput>;

  @Field(() => [ReportWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReportWhereUniqueInput],
  })
  disconnect?: Array<ReportWhereUniqueInput>;

  @Field(() => [ReportWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ReportWhereUniqueInput],
  })
  set?: Array<ReportWhereUniqueInput>;
}

export { ReportUpdateManyWithoutAssessmentResultsInput as ReportUpdateManyWithoutAssessmentResultsInput };
